...
void Visitor::visit(Visitable *p) {
  p -> accept(this);
}


void Visitor::visit_list(list<Visitable*> *v) {
  list<Visitable*>::iterator iter;
  for (iter = v->begin(); iter != v->end(); iter++)
    visit((*iter));
}


void Visitor::visit_children_of(Visitable *p) {
  p -> visit_children(this);
}
LatticeElemMap* CFVisitor::visit(Visitable *p, LatticeElemMap *in) {
  return p -> accept(this, in);
}


LatticeElemMap* CFVisitor::visit_list(list<Visitable*> *v, LatticeElemMap *in) {
  list<Visitable*>::iterator iter;
  for (iter = v->begin(); iter != v->end(); iter++)
    in = visit(*iter, in);
  return in;
...
LatticeElemMap* CFVisitor::visit_children_of(Visitable *p, LatticeElemMap *in) {
