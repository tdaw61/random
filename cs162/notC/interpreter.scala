...
object SemanticHelpers {
        ...
        def inject(prog:Program): Config = {
                        globalFunMap = prog.fds.foldLeft(FunMap())(( fm, fd) => fm + (fd.f -> fd))
                        Config(prog.t, Env() )
        }
...
object Interp {
        ...
        def eval(config:Config): Value = {
                ...
                case Assign(x, e) => config.env get x match {
                  ...
                  //var envn = Env()
                    //var envo = config.env
                      ...
                      //  val xvs = for ( VarBind(x, e) <- vbs )
                       // {
                        //  val a = alloc (evalTo(e))
                         // config.env = config.env + (x -> a)
                       // }
                       // evalTo(t)
                         //  config.env = envo

                        newEnv = vbs.foldLeft(config.env)(env,vbs) =>
                        {
                                val v = evalTo(e)
                                val a = alloc(evalTo(v));
                                env = env + (vb.x -> a)
                        }
                }
                case Call(f, es) => {
                        if (globalFunMap(f).xs.length!=es.length)
                                throw undefined("arguments and parameters don't match")
                        var envn = Env()
                        for (i<-0 until globalFunMap(f).xs.length)
                        {
                                envn+(
                                    globalFunMap(f).xs(i)->
                                    alloc( evalTo(es(i)) )
